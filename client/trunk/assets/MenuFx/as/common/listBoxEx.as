...
function _setItemMCPosY(mc:MovieClip, pos:Number)
{
    if(m_isVertical)
    {
        ...
        //if(Math.abs(pos-centerY))

        mc._x = m_basePosX-Math.ceil(Math.abs(pos-centerY)/3)

        mc.item_bg._height=101-Math.ceil(Math.abs(pos-centerY)/20)
    }
    else
    {
        mc._x = pos
...
//    debug("init free list size " + m_freeMCList.length)
    ...
    centerX=Math.ceil(this.hitZone_panel._width/2)
    centerY=Math.ceil(this.hitZone_panel._height/2)
...
function addToVisibleList(itemIndex, MCAddTo, addToHead)
{
    ...
    if(_scaleItemCallback)
    {
        _scaleItemCallback(mc)
    }
    return mc
}
